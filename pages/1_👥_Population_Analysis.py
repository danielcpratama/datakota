...
data = 'population'
data_df = get_cities(f"data/{data}_df.csv").drop(columns=['Unnamed: 0','KODE_KECAMATAN', 'KODE_KAB_KOTA', 'KODE_PROVINSI', 'POPULASI', 'JUMLAH_KK'])
...
with col1:
    ...
    analysis_key = list(join_df.columns)[11:]
...
with col2:
    ...
    with tab1:
        ...
        if unit_analysis == 'DESA_KELURAHAN':
            pivot_df = join_df[['KODE_KEL_DESA']+analysis_key]
            ...
            pivot_df = join_df.groupby(by=f'KODE_{unit_analysis}').sum()[analysis_key]
            ...
            pivot_df = join_df.groupby(by=f'KODE_{unit_analysis}').sum()[analysis_key].reset_index()
            pivot_df.KODE_PROVINSI = pivot_df.KODE_PROVINSI.astype(int).astype(str)
            pivot_gdf = pd.merge(gdf,pivot_df, how='left', on=f'KODE_PROVINSI')
        ...
        if normalizer == 'Per sqKM':
            ...
            pivot_gdf['val'] = pivot_gdf[sub_analysis]/pivot_gdf['POPULASI']
            pivot_gdf['RESULT'] = pivot_gdf['val'].round(2).astype(str) + f' {sub_analysis} {normalizer}'
            pivot_gdf = pivot_gdf.dropna()
        elif normalizer == 'Per Household':
            pivot_gdf['val'] = pivot_gdf[sub_analysis]/pivot_gdf['JUMLAH_KK']
            pivot_gdf['RESULT'] = pivot_gdf['val'].round(2).astype(str) + f' {sub_analysis} {normalizer}'
            pivot_gdf = pivot_gdf.dropna()
    ...
    with tab2: 
        with st.container(height=600, border=True):
            ...
            plot_df_d = plot_df_b.sort_values(by= 'val', ascending = True).head(10)
            ...
            fig_b.update_traces(marker_color = '#FF4B4B')
            fig_b.update_xaxes(title_text = axis_label)
            st.plotly_chart(fig_b)

            # plot bottom 10 analysis
            if normalizer != None: 
                plot_title = f'Bottom 10 {unit_analysis}, {city_analysis}, {province_analysis} with lowest {sub_analysis} {normalizer}'
                axis_label = f'% {sub_analysis} {normalizer}'
            else: 
                plot_title = f'Bottom 10 {unit_analysis}, {city_analysis}, {province_analysis} with lowest {sub_analysis}'
                axis_label = f'{sub_analysis}'
            
            fig_b = px.bar(plot_df_d, 
                        x = 'val',
                        y = f'NAMA_{unit_analysis}',
                        hover_data= place + ['RESULT'], 
                        title= plot_title,
