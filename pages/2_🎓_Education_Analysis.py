...
with col2:
    ...
    with tab2: 
        with st.container(height=600, border=True):
            ...
            plot_df_a = pd.DataFrame(pivot_gdf[analysis_key].sum()).reset_index().rename(columns={'index':'education_level', 0:'total'})
            ...
            fig_a = px.bar(plot_df_a, 
                    ...
                    y="education_level", 
                    orientation='h',
                    hover_data=["education_level", "total"],
                    height=400,
                    title=f'Demographic composition of {city_analysis}, {province_analysis} based on education level'
                    )
            fig_a.update_traces(marker_color = '#4F87BF')
            ...
            fig_b.update_traces(marker_color = '#4F87BF')
