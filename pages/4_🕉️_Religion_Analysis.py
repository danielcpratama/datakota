...
# set page config
st.set_page_config(
    page_title="Religion Analysis",
    layout="wide", 
    page_icon= "🕉️"
)
# ------------------------------------------------------------------------------
st.image('logo/logo.png', width=80)
st.title('Religion Analysis')
# ------------------------------------------------------------------------------
# SET UP SIDE BAR
with st.sidebar:
...
with col2:
    ...
    with tab2: 
        with st.container(height=600, border=True):
            ...
            plot_df_a = pd.DataFrame(pivot_gdf[analysis_key].sum()).reset_index().rename(columns={'index':'religion', 0:'total'})
            ...
            fig_a = px.bar(plot_df_a, 
                    ...
                    y="religion", 
                    orientation='h',
                    hover_data=["religion", "total"],
                    height=450,
                    title=f'Demographic composition of {city_analysis}, {province_analysis} based on religion composition'
                    )
            fig_a.update_traces(marker_color = '#3D6848')
            ...
            fig_b.update_traces(marker_color = '#3D6848')
