...
# set page config
st.set_page_config(
    page_title="Sex & Marriage Analysis",
    layout="wide", 
    page_icon= "👫🏻"
)
# ------------------------------------------------------------------------------
st.image('logo/logo.png', width=80)
st.title('Sex & Marriage Analysis')
# ------------------------------------------------------------------------------
# SET UP SIDE BAR
with st.sidebar:
...
with col2:
    ...
    with tab2: 
        with st.container(height=600, border=True):
            ...
            plot_df_a = pd.DataFrame(pivot_gdf[analysis_key].sum()).reset_index().rename(columns={'index':'marriage_status', 0:'total'})[2:]
            ...
            fig_a = px.bar(plot_df_a, 
                    ...
                    y="marriage_status", 
                    orientation='h',
                    hover_data=["marriage_status", "total"],
                    height=450,
                    title=f'Demographic composition of {city_analysis}, {province_analysis} based on marriage status'
                    )
            fig_a.update_traces(marker_color = '#B74195')
            ...
            fig_b.update_traces(marker_color = '#B74195')
